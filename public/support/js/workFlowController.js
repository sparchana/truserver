            ...
            candidateList.forEach(function (newCandidate) {
                ...
                var varColumn2 = function () {
                    if (app.currentView == "pre_screen_completed_view") {
                        ...
                        if($("#statusVal").val() == 2){
                            interviewAction = "Rescheduled on " + newCandidate.extraData.interviewSchedule + ". Awaiting candidate's response";
...
function confirmRejectInterview(){
    if($("#interviewStatusComments").val().trim() != ''){
        ...
        setInterviewStatus(globalCandidateId, 2, rescheduledDate, rescheduledSlot, $("#interviewStatusComments").val());
    } else{
...
function updateStatus(candidateId, jpId) {
    if($("#interview_status_" + candidateId).val() > 0){
        ...
        try {
            $.ajax({
                type: "POST",
                url: "/updateStatus/" + candidateId + "/" + jpId + "/" + $("#interview_status_" + candidateId).val(),
                data: false,
                contentType: false,
                processData: false,
