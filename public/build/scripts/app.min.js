...
function processDataSignUpSubmit(a){console.log("returedData :"+a.status),1==a.status?(returnedOtp=a.otp,$("#myRegistrationModal").modal("show"),$("#authMobile").val($("#candidateMobile").val()),$("#candidateFirstName").val(""),$("#candidateSecondName").val(""),$("#candidateMobile").val(""),$("#candidateEmail").val(""),document.getElementById("helpTextSignUp").innerHTML="Enter OTP sent on "+candidateMobile,$("#form_otp").show()):3==a.status?(alert("User already exists! Please login to continue"),window.location="/"):($("#errorMsg").show(),$("#candidateFirstName").val(""),$("#candidateMobile").val(""),$("#candidateEmail").val(""))}function processDataAddAuth(a){console.log("returedData :"+a.status),1==a.status?(localStorage.setItem("mobile","+91"+candidateMobile),localStorage.setItem("name",a.candidateFirstName),localStorage.setItem("lastName",a.candidateLastName),localStorage.setItem("id",a.candidateId),localStorage.setItem("leadId",a.leadId),localStorage.setItem("assessed",a.isAssessed),console.log(a.candidateId),window.location="/dashboard"):$("#errorMsg").show()}var returnedOtp,candidateMobile;$(function(){$("#form_signup_candidate").submit(function(a){a.preventDefault();var e=1,t=$("#candidateFirstName").val(),d=$("#candidateSecondName").val(),o=$("#candidateMobile").val(),i=validateName(t),n=validateName(d),s=validateMobile(o),r=$("#candidateLocality").val(),l=$("#candidateJobPref").val();if(0==i?(alert("Please Enter First Name"),e=0):0==n?(alert("Please Enter your Last Name"),e=0):0==s?(alert("Enter a valid mobile number"),e=0):1==s?(alert("Enter 10 digit mobile number"),e=0):""==r?(alert("Please Enter your Job Localities"),e=0):""==l&&(alert("Please Enter the Jobs you are Interested"),e=0),1==e){candidateMobile=o,document.getElementById("registerBtn").disabled=!0;try{var c,u=[],m=[],p=$("#candidateJobPref").val().split(","),v=$("#candidateLocality").val().split(",");for(c=0;c<p.length;c++)u.push(parseInt(p[c]));for(c=0;c<v.length;c++)m.push(parseInt(v[c]));$("#alreadyMsgCandidate").hide();var g={candidateFirstName:t,candidateSecondName:d,candidateMobile:o,candidateLocality:m,candidateJobPref:u};$.ajax({type:"POST",url:"/signUp",contentType:"application/json; charset=utf-8",data:JSON.stringify(g),success:processDataSignUpSubmit})}catch(b){console.log("exception occured!!"+b)}}})}),$(function(){$("#form_otp").submit(function(a){a.preventDefault();var e=$("#candidateOtp").val();e==returnedOtp?($("#incorrectOtpMsg").hide(),$("#form_otp").hide(),$("#form_auth").show(),$("#errorMsg").hide(),$("#incorrectMsg").hide()):($("#incorrectOtpMsg").show(),$("#errorMsg").hide(),$("#candidateOtp").val(""))})}),$(function(){$("#form_auth").submit(function(a){a.preventDefault();var e=$("#candidatePassword").val(),t=validatePassword(e);if(0==t)alert("Please set min 6 chars for password");else if(1==t)alert("Password cannot have blank spaces. Enter a valid password");else{document.getElementById("btnSubmit").disabled=!0;try{var d=$("#candidatePassword").val(),o=candidateMobile,i={candidatePassword:d,candidateAuthMobile:o};console.log("userMobile: "+o),$.ajax({type:"POST",url:"/addPassword",contentType:"application/json; charset=utf-8",data:JSON.stringify(i),success:processDataAddAuth})}catch(n){console.log("exception occured!!"+n)}}})});
function validateMobile(t){var e=/^[7-9]{1}[0-9]{9}$/i;return t.length>0&&e.test(t)===!1?0:0==t.length?1:2}function validateName(t){var e=/^[a-zA-Z]+$/,a=/^[ ]+$/,n=/[^\w\s]/gi,s=/^[a-zA-Z0-9]+$/;return e.test(t)?1:s.test(t)?0:a.test(t)?0:n.test(t)?0:1}function validatePassword(t){var e=/^[ ]+$/;return t.length<6?0:e.test(t)?(console.log("Password with blank spaces"),1):2}