 ...
 *
 * c.f copied from https://sourceforge.net/p/opencsv/source/ci/master/tree/src/main/java/com/opencsv/bean/CsvToBean.java
 *
 * modified exception handling, introduced NumberFormatException message capture inside parse() method,
 *
 *
...
public class CsvToBean<T> extends AbstractCSVToBean {
        ...
        try {
            ...
            while (null != (line = csvReader.readNext())) {
                ...
                try {
                    ...
                    CsvDataTypeMismatchException mcsve = new CsvDataTypeMismatchException(nfe.getMessage());
                    mcsve.setLineNumber(lineProcessed);
                    getCapturedExceptions().add(mcsve);
                    // if file read is more than 100 lines and if exceptions are more than 10% of the total lines read,
                    // then reject file
                    if( csvReader.getLinesRead()> 100 && getCapturedExceptions().size() > csvReader.getLinesRead()*0.1 ) {
                        throw new RuntimeException("Too many exceptions encountered");
                    }
                }
    ...
    private void processLine(MappingStrategy<T> mapper, CsvToBeanFilter filter, String[] line, List<T> list)
            ...
            CsvRequiredFieldEmptyException, CsvConstraintViolationException, NumberFormatException {
